...
class TokenService{
    ...
    async findToken(refreshToken){
        ...
        const tokenData = await tokenModel.findOne({refreshToken}); // вызываем функцию findOne() у таблицы Token в базе данных,передаем туда объект с полем refreshToken,то есть будет найден объект с полем refreshToken и значением таким же,как и параметр этой функции findToken и этот найденный объект помещаем в переменную tokenData
