...
class UserController {
    ...
    async updateProductRatingCart(req, res, next) {
        ...
        try {
            ...
            const productCartFounded = await cartMealModel.find({ usualProductId: product._id });  // находим все товары(блюда) корзины,у которых поле usualProductId равно полю _id у product(весь объект тела запроса,то есть обычный объект товара(блюда) каталога),находим все эти товары(блюда) корзины,так как одинаковые товары могут быть у разных пользователей в корзине(только эти товары могут отличаться количеством,в зависимости от того,сколько выбрал пользователь положить их в корзину)
            ...
            if (productCartFounded) {
                ...
                await cartMealModel.updateMany({ usualProductId: product._id }, { rating: product.rating }); // обновляем все объекты товаров(блюд) корзины с помощью updateMany(), у которых поле usualProductId равно полю _id у product(весь объект тела запроса,то есть обычный объект товара(блюда) каталога),обновляем у них поле rating на значение поля rating у product(весь объект тела запроса,то есть обычный объект товара(блюда) каталога)
            }
    ...
    async deleteMealCatalog(req, res, next) {
        ...
        try {
            ...
            if (commentsForDeletedMeal) {
                ...
                await commentModel.deleteMany({ productNameFor: mealCatalog.name }); // удаляем все комментарии с помощью deleteMany(), у которых productNameFor равен name товара,который хотим удалить,который взяли из тела запроса

            }

            // ищем все объекты товаров(блюд) в корзине, у которых поле name равно полю name объекта товара(блюда),который хотим удалить(mealCatalog.name)
            const deletedMealsCart = await cartMealModel.find({ name: mealCatalog.name });

            // если deletedMealsCart true,то есть товар,который нужно удалить есть в корзине пользователя(или в корзине у нескольких пользователей),то его удаляем из корзины пользователя(или нескольких пользователей) в базе данных
            if (deletedMealsCart) {

                await cartMealModel.deleteMany({ name: mealCatalog.name }); // удаляем все объекты товаров(блюд) в корзине у которых поле name равно полю name объекта товара,который хотим удалить(mealCatalog.name),указываем условие именно по полю name,так как нужно удалить все объекты этого товара у всех пользователей,потому что поле name у них одинаковое сохраняется в базе данных в таблице(сущности) cartMealModel,но разные id и forUser(это поле нужно,чтобы показать к какому пользователю этот товар принадлежит),поэтому так удаляем их у всех пользователей

            }

            const deletedMealCatalog = await mealModel.findByIdAndDelete(mealCatalog._id); // находим и удаляем объект товара каталога с помощью findByIdAndDelete(), у которого id равен _id(указываем здесь нижнее подчеркивание перед id,так как оно по дефолту идет у объектов базы данных mongodb с нижним подчеркиванием) товара,который хотим удалить,который взяли из тела запроса(mealCatalog._id)


            const filePath = `${path.resolve()}\\static\\${mealCatalog.image}`; // помещаем путь до файла,который хотим удалить в переменную filePath(здесь path.resolve() - берет текущую директорию(в данном случае директорию до \serverNodeJsMongoDb) потом через слеши наша папка static в которой мы храним все скачанные файлы с фронтенда и еще через слеши указываем название файла,берем название файла картинки у поля image у mealCatalog(у товара,который хотим удалить,который взяли из тела запроса))
    ...
    async getAdminFields(req, res, next) {
        ...
        try {
            ...
            const adminFields = await adminFieldsModel.findOne({});  // ищем объект у таблицы(сущности) adminFields в базе данных и помещаем его в переменную adminFields,передаем в findOne() пустой объект,чтобы найти первую попавшуюся запись у сущности adminFields(так как она там и так одна для всех полей,то ее так и находим),находим этот объект(он будет один со всеми нужными полями,так как мы его так создали в базе данных),в нем будут поля со значением текстов и разных полей на сайте,ищем и будем отображать потом на сайте,чтобы потом мог админ изменять их в базе данных
    ...
    async changeAdminFields(req, res, next) {
        ...
        try {
            ...
            // если adminFields.phoneNumber.trim().length > 3,то есть поле номера телефона,которое мы взяли из тела запроса,отфильтрованное по пробелам(то есть пробелы убрались из этого поля с помощью trim()) по количеству символов больше 3,то изменяем объект адми полей,в другом случае показываем ошибку
            if(adminFields.phoneNumber.trim().length > 3){
                
                const foundedAdminFields = await adminFieldsModel.findOne({}); // ищем объект у таблицы(сущности) adminFields в базе данных и помещаем его в переменную adminFields,передаем в findOne() пустой объект,чтобы найти первую попавшуюся запись у сущности adminFields(так как она там и так одна для всех полей,то ее так и находим),находим этот объект(он будет один со всеми нужными полями,так как мы его так создали в базе данных),в нем будут поля со значением текстов и разных полей на сайте,ищем и будем отображать потом на сайте,чтобы потом мог админ изменять их в базе данных

                foundedAdminFields.phoneNumber = adminFields.phoneNumber; // изменяем поле phoneNumber у этого найденного объекта для админ полей(foundedAdminFields) на значение поля phoneNumber у adminFields(объект тела запроса)
                ...
                return next(ApiError.BadRequest('Phone number must be more than 3 characters and without letters')); // бросаем ошибку,возвращаем с помощью return нашу функцию next(это наш errorMiddleware) и передаем туда ошибку с помощью нашего ApiError и в BadRequest(наша функция для ошибки) передаем сообщение ошибки,не используем тут throw ApiError,так как это родительский компонент ошибки(то есть если и будет ошибка,то она будет только тут) и тут есть сразу функция next в параметрах этой функции changeAdminFields,указываем return,чтобы код ниже не работал,если будет ошибка,но можно тут указать и throw ApiError
            }
