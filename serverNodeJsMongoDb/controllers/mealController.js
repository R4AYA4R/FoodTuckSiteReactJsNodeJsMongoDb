...
class MealController {
    ...
    async createMealCart(req, res, next) {

        // оборачиваем в блок try catch,чтобы отлавливать ошибки
        try {

            const mealCart = req.body; // достаем(деструктуризируем) из тела запроса весь объект запроса со всеми полями,которые мы передали с фронтенда(не используем здесь деструктуризацию типа деструктурировать из req.body {mealCart} в квадратных скобках,так как просто берем все тело запроса,то есть весь объект тела запроса,а не отдельные поля)

            const createdMealCart = await cartMealModel.create({...mealCart}); // создаем объект товара(блюда) корзины в базе данных,разворачивая весь объект mealCart(это весь объект тела запроса),вместо ...mealCart будут подставлены все поля с их значениями,которые есть в объекте тела запроса

            return res.json(createdMealCart); // возвращаем созданный объект товара(блюда) корзины на фронтенд
    ...
    async getAllMealsCart(req, res, next) {

        // оборачиваем в блок try catch,чтобы отлавливать ошибки
        try {

            const { userId } = req.query; // берем из url(строки) запроса параметр userId,чтобы получить все товары(блюда) корзины для определенного авторизованного пользователя

            
            const mealsCart = await cartMealModel.find({ forUser:userId }); // получем все объекты товаров(блюд) корзины у которых поле forUser равно параметру userId который мы взяли из query параметров url(строки) запроса,то есть получаем все объекты товаров(блюд) для определенного авторизованного пользователя

            return res.json(mealsCart); // возвращаем найденные товары(блюда) корзины на фронтенд
    ...
    async updateCartMeal(req, res, next) {

        // оборачиваем в блок try catch,чтобы отлавливать ошибки
        try {

            const cartMeal = req.body; // достаем(деструктуризируем) из тела запроса весь объект запроса со всеми полями,которые мы передали с фронтенда(не используем здесь деструктуризацию типа деструктурировать из req.body {cartMeal} в квадратных скобках,так как просто берем все тело запроса,то есть весь объект тела запроса,а не отдельные поля)

            const cartMealFounded = await cartMealModel.findById(cartMeal._id); // находим объект блюда в базе данных по id,который взяли у объекта тела запроса cartMeal(это будет объект с обновленными данными товара(блюда) корзины,если пользователь изменил эти данные товара(блюда) корзины на фронтенде,если пользователь эти данные товара(блюда) не изменил,то это будет объект товара(блюда) такой же,как и был)

            // если cartMealFounded.amount не равно cartMeal.amount(то есть если количество найденного товара(блюда) корзины в базе данных(cartMealFounded) не равно количеству товара(блюда) корзины cartMeal(который мы передали с фронтенда),то тогда изменяем данные товара корзины,в другом случае возвращаем текст типа с ошибкой на клиент(фронтенд))
            if(cartMealFounded.amount !== cartMeal.amount){

                cartMealFounded.amount = cartMeal.amount; // изменяем поле amount у cartMealFounded(у этого объекта товара(блюда) корзины,который мы нашли в базе данных) на значение поля amount у cartMeal(объекта товара(блюда),который мы передали с фронтенда)

                cartMealFounded.totalPrice = cartMeal.totalPrice; // изменяем поле totalPrice у cartMealFounded(у этого объекта товара(блюда) корзины,который мы нашли в базе данных) на значение поля totalPrice у cartMeal(объекта товара(блюда),который мы передали с фронтенда)

                await cartMealFounded.save(); // сохраняем обновленный объект товара(блюда) в базе данных

                return res.json(cartMealFounded); // возвращаем обновленный объект товара(блюда) на клиент(фронтенд)

            } else {

                return res.json(`Данные товара корзины с id = ${cartMealFounded.id} не были изменены`); // вместо ошибки просто возвращаем текст с сообщением,в данном случае просто,чтобы не было куча ошибок в консоли,так как это не критичная ошибка,а просто больше как уведомление
    ...
    async deleteCartMeal(req, res, next) {

        // оборачиваем в блок try catch,чтобы отлавливать ошибки
        try {

            const { mealId } = req.params; // берем mealId из параметров запроса(мы указали этот динамический параметр mealId в url к эндпоинту,поэтому можем его взять из req.params)

            const deletedCartMeal = await cartMealModel.findByIdAndDelete(mealId); // находим объект товара(блюда) у которого id равен параметру mealId(который мы взяли из url(строки) запроса) и удаляем его с помощью функции findByIdAndDelete(),если здесь использовать функцию deleteOne() и удалять объект товара(блюда) по id,то не будет работать,потому что по id так не найдет,поэтому в данном случае используем функцию findByIdAndDelete()

            console.log(deletedCartMeal);

            return res.json(deletedCartMeal); // возвращаем на клиент(фронтенд) удаленный объект товара
